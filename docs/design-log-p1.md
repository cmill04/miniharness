...
First of all inside the conversation class all five elements of the rule of 5 are there: copy constructor, destructor, assignment operator, move constructor, and move assignment. These elements are also created to avoid the "common pitfalls". The copy creates a deep copy rather than a shallow copy and this is tested in test 5 of the test file. The test shows that addresses are not the same when copied. The move constructor is also tested by showing that the pointer is copied and the old object is cleared out. The assignment operator allows for safe copying by checking if (this == &other) this would just waste work as nothing is changing. The destructor calls delete [] data_ unconditionally which protects against an empty Conversation because delete will do nothing when the pointer is null.
